...
require 'json'
#Get the functions for JSON files
...
while newUser == false
    ...
    #Checking to see if user is returning, or visiting for the first time. If returning, checking details as well
    if returning == "yes" || returning == "y"
        puts "Please enter your username: "
        while returningUser == false
            username = gets.chomp.downcase
            #Check if user file exists, if it does, it means user exists and passowrd can be checked
            if File.file?("#{username}.txt")
                userHashes = read_from_file("#{username}.txt")
        ...
        while unique == false
            username = gets.chomp.downcase
            if File.file?("#{username}.txt")
                puts "Username already exists. Please select another. "
            else
                puts "Thank you. "
                puts "Please select a password. "
                password = gets.chomp.downcase
                File.new("#{username}.txt", 'w').close
                user_info = {
                    'username' => username,
                    'password' => password,
                    'balance' => 0.00
...
# Start of the actual app, variable so the user can quit
session = true
#Setting variables balance and history
balance = 0
history = []
